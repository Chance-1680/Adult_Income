...
```{python}
adult_df['workclass'].value_counts(normalize=True).rename_axis('unique values')
```
...
```{python}
total_per_age_group = adult_df_income_age.groupby('age_group')['total_by_age'].transform('sum')
total_per_age_group
```

```{python}
adult_df_income_age['percentage'] = (adult_df_income_age['total_by_age'] / total_per_age_group) * 100
adult_df_income_age
```
...
fig.update_layout(template="presentation",
                  ...
                  plot_bgcolor = "rgba(0, 0, 0, 0)",
                  title_font_size=15,  
                  xaxis=dict(title_font_size=13, tickfont_size=12),  
                  yaxis=dict(title_font_size=13, tickfont_size=12), # Y-axis title and tick font sizes
...
fig.update_traces(texttemplate='%{text:.2f}%', textposition = 'auto') # Changed to 'auto'
fig.update_layout(template = "presentation",
                  xaxis_title = 'Native Region', # Capitalized 'Native Region'
                  yaxis_title = 'Percentage of Population', # Capitalized 'Percentage of Population'
                  legend_title = dict(text='Income Level'),
                  paper_bgcolor="rgba(0, 0, 0, 0)",
                  plot_bgcolor ="rgba(0, 0, 0, 0)",
                  title_font_size=20,
                  xaxis=dict(title_font_size=13, tickfont_size=12, tickangle=45, automargin=True), # Added automargin
                  yaxis=dict(title_font_size=13, tickfont_size=12),
                  legend=dict(font=dict(size=12)),
                  margin=dict(l=40, r=40, t=50, b=150) # Increased bottom margin significantly
                 )
fig.show()
...
fig.update_traces(texttemplate='%{text:.2f}%', textposition = 'auto') # Changed to 'auto' for potentially better placement
fig.update_layout(template = "presentation",
                  xaxis_title = 'Race',
                  yaxis_title = 'Percentage of Population', # Capitalized 'Percentage of Population'
                  legend_title = dict(text='Income Level'),
                  paper_bgcolor="rgba(0, 0, 0, 0)",
                  plot_bgcolor ="rgba(0, 0, 0, 0)",
                  title_font_size=20, # Increase main title font size
                  xaxis=dict(title_font_size=13, tickfont_size=12), # X-axis title and tick font sizes
                  yaxis=dict(title_font_size=13, tickfont_size=12), # Y-axis title and tick font sizes
                  legend=dict(font=dict(size=12)), # Legend font size
                  margin=dict(l=40, r=40, t=50, b=40) # Adjust margins for overall layout
                 )
fig.show()
```
...
## 10. Income Distribution by Education Level and Ocuupation grouped
...
fig = px.bar(
    ...
    color = 'income',
    orientation='h',
    title = f'Top {num} Education and Occupation Group Combinations by Income Level', # Clarified title
    height= 500,
    width= 1100,
    color_discrete_sequence=px.colors.sequential.RdBu,
    text = 'total'
...
fig.update_layout(template = "presentation",
                  ...
                  yaxis_title = 'Education | Occupation Group', # Capitalized 'Group'
                  legend_title = dict(text='Income Level'),
                  margin = dict(l=450, r=50, t=70, b=50), # Adjusted top margin slightly
                  title_font_size=20, # Increase main title font size
                  xaxis=dict(title_font_size=16, tickfont_size=12), # X-axis title and tick font sizes
                  yaxis=dict(title_font_size=16, tickfont_size=12), # Y-axis title and tick font sizes
                  legend=dict(font=dict(size=12)) # Legend font size
                 )
fig.update_traces(textposition = 'inside', textfont=dict(size=10)) # Adjust text font size inside bars
fig.show()
